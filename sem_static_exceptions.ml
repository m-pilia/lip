...
let rec sem_exp e env = match e with
  ...
  | Try (e, handler_ide, handler_exp) -> 
       (match sem_exp e (bind env handler_ide (DExcept(handler_ide))) with
       | DExcept (ide) when ide = handler_ide -> sem_exp handler_exp env
       | v -> v)
  | Raise (handler_ide) -> try 
                            applyenv env handler_ide
                           with
                            UnboundException (ide) -> failwith ("Exception: " ^ ide)
